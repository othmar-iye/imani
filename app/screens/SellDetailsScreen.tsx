...
export default function ProfileSettingsScreen() {
  ...
  const fetchUserProfile = async (): Promise<UserProfile | null> => {
    ...
    try {
      console.log('🔍 Chargement profil utilisateur...', user.id);
      ...
      if (error) {
        ...
        if (error.code === 'PGRST116') {
          console.log('ℹ️  Profil non trouvé (première connexion)');
        }
        return null;
      }
  ...
  useEffect(() => {
    if (userProfile) {
      console.log('🔄 Initialisation des données du profil...');
  ...
  const uploadImageToStorage = async (imageUri: string, path: string): Promise<string | null> => {
    try {
      console.log('📤 Début upload image:', { imageUri, path });
      ...
      console.log('📄 Préparation upload:', { filename, uniqueFileName });
      ...
      console.error('❌ Erreur upload image complète:', error);
      return null;
    }
  };
  ...
  const saveProfileData = async (formData: ProfileFormData): Promise<boolean> => {
    ...
    try {
      ...
      if (formData.birthDate) {
        ...
        console.log('📅 Date formatée:', birthDateFormatted);
      }
      ...
      console.log('🎯 Statut vérification:', verificationStatus);
  ...
  const handleSave = async () => {
    console.log('🔄 Début validation sauvegarde...');
    ...
    if (!localProfileData.profilePicture) {
      console.log('❌ Photo de profil manquante');
      Alert.alert(
        t('photoRequired'),
    ...
    console.log('✅ Validation réussie - Lancement sauvegarde');
